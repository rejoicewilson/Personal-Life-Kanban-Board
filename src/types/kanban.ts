...
export function createSeedBoard(userId: string) {
  const categories: Category[] = [
    { id: `${userId}:work`, name: 'Work', color: 'sky' },
    { id: `${userId}:personal`, name: 'Personal', color: 'violet' },
    { id: `${userId}:urgent`, name: 'Urgent', color: 'rose' },
  ]

  const categoryIds = {
    work: categories.find((category) => category.name === 'Work')?.id ?? `${userId}:work`,
    personal: categories.find((category) => category.name === 'Personal')?.id ?? `${userId}:personal`,
    urgent: categories.find((category) => category.name === 'Urgent')?.id ?? `${userId}:urgent`,
  }
